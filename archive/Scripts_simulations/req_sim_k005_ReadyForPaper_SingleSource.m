% EXP ID: REQ-SIM-K005-ReadyForPaper-SingleSource
...
medium.sound_speed_compression = 2 * ones(Nx, Ny);   % [m/s] realistic compressional speed
...
% Build Nt×Ns signals (vectorized)                      % desired bandwidth in Hz

D = 0.5;            % duty cycle as fraction
D = min(max(D, 0.01), 0.99);     % limit between 1% and 99%

S = square(2*pi*source_freq*t + phi(k_id).', 100*D);
...
% Optional: save
filename = ['req_sim_k005_rfp_SS_' num2str(source_freq) 'Hz'];
