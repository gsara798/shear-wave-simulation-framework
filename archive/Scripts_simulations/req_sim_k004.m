...
medium.sound_speed_compression = 30 * ones(Nx, Ny);   % [m/s] realistic compressional speed
...
inc_radius = 25;                  % radius in grid points
inc_cx = round(Nx/2);
inc_cy = round(Ny/2);
...
medium.sound_speed_compression(inc_mask) = 30;  % m/s (background 1500)
medium.density(inc_mask)                = 1000;  % kg/m^3 (background 1000)
...
sws = medium.sound_speed_shear;   % (Nx x Ny)
...
% rng(0);                 % enable for reproducibility
num_act    = 128;           % total actuators
radius_pts = 14;           % disc radius in grid points

margin=20;

% safe border line indices (inner edge of physical region)
xL = margin + 1;
xR = Nx - margin;
yT = margin + 1;
yB = Ny - margin;

% avoid corners so discs stay inside physical region
m = radius_pts + 4;

% pick border for each actuator (1=top,2=bottom,3=left,4=right)
border_id = randi(4, [num_act,1]);
...
for k = 1:num_act
    disc = makeDisc(Nx, Ny, cx_list(k), cy_list(k), radius_pts);  % numeric 0/1
...
roi_lat_pts   = 120;   % lateral width (# points)
roi_depth_pts = 120;   % axial depth (# points)
...
tx_y = 20;                    % just below air boundary (top)
...
try
    ...
    E=MException('req_sim_k004','%s',pwd);
catch E
...
Nt = size(ux_s,3);
Nt_target = min(10*source_freq, Nt);
...
% Optional: save
filename = ['req_sim_k004_' num2str(num_act) 'sources' num2str(source_freq) 'Hz'];
%save(filename, "ux_total", "ux_p", "ux_s", "dinf", "source_freq", '-v7.3');
save(filename, "ux_p", "ux_s", "dinf", "source_freq", '-v7.3');
