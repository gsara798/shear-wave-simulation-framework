...
function testExpandsIndexedObjectProperty(testCase)

campaign = struct();
campaign.schema_version = "1.0";
campaign.campaign_name = "indexed_object_expansion";
...
runs = kwsim.campaigns.expandCampaign(campaign_file);

verifyEqual(testCase, numel(runs), 2);

verifyEqual(testCase, ...
    runs(1).config.geometry.objects(1).cs_m_s, ...
    2.5);

verifyEqual(testCase, ...
    runs(2).config.geometry.objects(1).cs_m_s, ...
    4.0);

verifyEqual(testCase, ...
    runs(1).config.geometry.objects(1).cp_m_s, ...
    20.0);

verifyEqual(testCase, ...
    runs(2).config.geometry.objects(1).cp_m_s, ...
    20.0);
