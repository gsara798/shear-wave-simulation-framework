...
verifyEqual(testCase, pyMeasured, expectedTangentialSlowness, ...
    RelTol=0.12, ...
    sprintf("Tangential slowness was %.6g s/m; expected %.6g s/m.", ...
    pyMeasured, expectedTangentialSlowness));
verifyEqual(testCase, theta2MeasuredDeg, theta2ExpectedDeg, ...
    AbsTol=4.0, ...
    sprintf("Transmitted angle was %.3f deg; Snell predicts %.3f deg.", ...
    theta2MeasuredDeg, theta2ExpectedDeg));
