...
verifyEqual(testCase, sample.wavefield.data_zx, ...
...
verifyEqual(testCase, sample.truth.cs_map_zx, result.truth.cs_m_s_zx);
verifyEqual(testCase, sample.truth.k_map_zx, ...
    2*pi*500 ./ result.truth.cs_m_s_zx);
verifyEqual(testCase, sample.extraction.method, "native_2d_sensor_roi");
...
function testCreatesVolumetric3DSample(testCase)
...
verifyEqual(testCase, sample.wavefield.data_zyx, ...
    result.fields.displacement.z_total_zyx);
verifyEqual(testCase, sample.truth.cs_map_zyx, result.truth.cs_m_s_zyx);
verifyEqual(testCase, sample.truth.material_id_zyx, ...
    result.truth.material_id_zyx);
verifyEqual(testCase, sample.extraction.method, "native_3d_sensor_roi");
...
verifyEqual(testCase, sample.measurement.axis_xyz, [0, 0, 1]);
verifySize(testCase, sample.wavefield.data_zyx, [3, 5, 4]);
...
result.fields.velocity.axial_total_zx = ...
    1i*2*pi*frequencyHz .* result.fields.displacement.axial_total_zx;
...
sample = kwsim.samples.buildWavefieldSample(synthetic2DResult());
...
Nz = 3; Nx = 4;
field = complex(reshape(1:(Nz*Nx), Nz, Nx),0.25);
result = struct();
