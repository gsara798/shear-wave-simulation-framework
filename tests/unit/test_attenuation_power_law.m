...
function testReferenceConfigurationIsDeterministic(testCase)
first = attenuationFixture( ...
    ShearAlphaRefDbCm=1.7, ...
    ShearReferenceFrequencyHz=450, ...
    ShearPowerY=1.1, ...
    Seed=42);
second = attenuationFixture( ...
    ShearAlphaRefDbCm=1.7, ...
    ShearReferenceFrequencyHz=450, ...
    ShearPowerY=1.1, ...
    Seed=42);
...
function cfg = attenuationFixture(options)
arguments
...
cfg = kwsim.two_d.defaultConfig();
cfg.scenario = "attenuation_unit_fixture";
...
cfg.grid.cfl = 0.025;
cfg.attenuation.enabled = true;
cfg.attenuation.materials = kwsim.materials.makeAttenuationMaterial( ...
    1, ...
    ShearAlphaRefDbCm=options.ShearAlphaRefDbCm, ...
    ShearReferenceFrequencyHz=options.ShearReferenceFrequencyHz, ...
    ShearPowerY=options.ShearPowerY, ...
    CompressionAlphaRefDbCm=0.1, ...
    CompressionReferenceFrequencyHz=500, ...
    CompressionPowerY=1.2);
end
...
cfg.geometry.objects = kwsim.geometry.two_d.makeCircleObject( ...
    ...
    8e-3, 2, 3.0, 1020, "attenuation_test_inclusion");
end
