...
empty_record.condition_id = "";
empty_record.realization_id = NaN;
empty_record.run_id = "";
...
for index = 1:numel(runs)
    ...
    records(index).condition_id = runs(index).condition_id;
    records(index).realization_id = runs(index).realization_id;
...
if ~isAbsolutePath(root_directory)
    campaign_directory = fileparts( ...
        string(expansion.campaign_file));
...
root_directory = canonicalPath(root_directory);
...
marker.condition_id = run.condition_id;
marker.realization_id = run.realization_id;
