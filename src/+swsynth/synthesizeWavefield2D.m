...
parentSourceXYZ = [double(xSource(:)), double(ySource(:)), ...
    double(zSource(:))];
apertureDefinition = swsynth.sources.buildCoherentAperture( ...
    parentSourceXYZ(1,:), cfg.sources.aperture);
useFiniteAperture = cfg.sources.aperture.model == "line_segment";

if useFiniteAperture
    elementOffsetsXYZ = ...
        apertureDefinition.node_offsets_m(:) .* ...
        apertureDefinition.axis_xyz;
    sourceElementXYZ = reshape(parentSourceXYZ, N, 1, 3) + ...
        reshape(elementOffsetsXYZ, 1, [], 3);
else
    sourceElementXYZ = zeros(0, 0, 3);
end
            ...
            case "local_k_distance"
                parfor (zIndex = 1:Nz, parallelFlag(useParallel))
                    ...
                    for directionIndex = 1:N
                        distance = zeros(0, 1, "single");
                        geometricAmplitude = zeros(0, 1, "single");
                        if ~useFiniteAperture
                            distance = sqrt( ...
                        ...
                        if cfg.sources.radiation.model == ...
                            ...
                            if useFiniteAperture
                                apertureContribution = ...
                                    complex(zeros(Nx, 1, "single"));
                                for elementIndex = ...
                                        1:apertureDefinition.node_count
                                    elementSourceXYZ = squeeze( ...
                                        sourceElementXYZ( ...
                                            directionIndex, ...
                                            elementIndex, :)).';
                                    elementDistance = sqrt( ...
                                        (xColumn - elementSourceXYZ(1)).^2 + ...
                                        (observationY - elementSourceXYZ(2)).^2 + ...
                                        (zValue - elementSourceXYZ(3)).^2);
                                    elementAmplitude = ...
                                        computeGeometricAmplitude( ...
                                            elementDistance, ...
                                            cfg.amplitude.geometric_decay_exponent);
                                    elementProjection = ...
                                        computeRadiationProjection( ...
                                            cfg, elementSourceXYZ, ...
                                            xColumn, observationY, zValue);
                                    apertureContribution = ...
                                        apertureContribution + ...
                                        single(apertureDefinition.node_weights( ...
                                            elementIndex)) .* ...
                                        elementProjection .* ...
                                        exp(1i * (localK .* elementDistance)) .* ...
                                        elementAmplitude;
                                end
                                column = column + weights(directionIndex) .* ...
                                    apertureContribution;
                            else
                                radiationProjection = ...
                                    computeRadiationProjection( ...
            ...
            case "straight_ray_numerical"
                ...
                parfor (directionIndex = 1:N, parallelFlag(useParallel))
                    ...
                    if cfg.sources.radiation.model == ...
                            "point_force_shear_far_field"
                        if useFiniteAperture
                            apertureContribution = complex( ...
                                zeros(pointCount, 1, "single"));
                            for elementIndex = ...
                                    1:apertureDefinition.node_count
                                elementSourceXYZ = squeeze( ...
                                    sourceElementXYZ( ...
                                ...
                                elementTravelTimeS = ...
                                    swsynth.integrateStraightRayTravelTime( ...
                                        cfg, elementSourceXYZ, targetXYZ);
                                elementDistance = sqrt(sum( ...
                                    (targetXYZ - elementSourceXYZ).^2, 2));
                                elementAmplitude = ...
                                    computeGeometricAmplitude( ...
                                        elementDistance, amplitudeExponent);
                                elementPhase = single( ...
                                    omega .* elementTravelTimeS);
                                elementProjection = ...
                            ...
                            distance = sqrt(sum( ...
                                (targetXYZ - sourceXYZ).^2, 2));
                            geometricAmplitude = computeGeometricAmplitude( ...
                                distance, amplitudeExponent);
...
if cfg.propagation.model == "spherical_wave"
    wavefield.sources = struct( ...
        ...
        "aperture_model", apertureDefinition.model, ...
        "aperture_span_m", apertureDefinition.realized_span_m, ...
        "aperture_axis_xyz", apertureDefinition.axis_xyz, ...
        "aperture_node_spacing_m", ...
            apertureDefinition.node_spacing_m, ...
        "aperture_node_count", apertureDefinition.node_count, ...
        "aperture_node_offsets_m", ...
            apertureDefinition.node_offsets_m, ...
        "aperture_node_weights", ...
            apertureDefinition.node_weights);
else
    wavefield.sources = struct( ...
        "x_m", [], ...
        "y_m", [], ...
