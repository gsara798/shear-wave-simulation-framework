...
exportOne(@() simviz.plotSws(sample,Visible=visibility),files.sws,options.ResolutionDPI);
exportOne(@() simviz.plotField(sample,Visible=visibility),files.field,options.ResolutionDPI);
exportOne(@() simviz.plotPhase(sample,Visible=visibility),files.phase,options.ResolutionDPI);
exportOne(@() simviz.plotAmplitude(sample,Visible=visibility),files.amplitude,options.ResolutionDPI);
...
if isfield(sample,"directions") && isfield(sample.directions,"xyz") && ~isempty(sample.directions.xyz)
    ...
    exportOne(@() simviz.plotDirections(sample,Visible=visibility),files.directions,options.ResolutionDPI);
end
...
if double(sample.spatial_dimension) == 3
    ...
    exportOne(@() simviz.plotSwsCrossplanes(sample,Visible=visibility), ...
        files.sws_crossplanes,options.ResolutionDPI);
    exportOne(@() simviz.plotAmplitudeCrossplanes(sample,Visible=visibility), ...
        files.wavefield_amplitude_crossplanes,options.ResolutionDPI);
    exportOne(@() simviz.plotPhaseCrossplanes(sample,Visible=visibility), ...
        files.wavefield_phase_crossplanes,options.ResolutionDPI);
    exportOne(@() simviz.plotFieldCrossplanes(sample,Visible=visibility), ...
