...
arguments
    sample (1,1) struct
    options.Title (1,1) string = "Wavefield real part"
    options.Visible (1,1) string = "off"
end
[x,z,U,~,description] = simviz.displayPlane(sample);
values = real(U);
limit = max(abs(values),[],"all"); if ~(isfinite(limit) && limit>0), limit=1; end
fig = figure("Visible",char(options.Visible),"Color","w","Position",[100 100 760 620]);
ax = axes(fig); imagesc(ax,x,z,values); axis(ax,"image"); set(ax,"YDir","normal");
xlabel(ax,"x [mm]"); ylabel(ax,"z [mm]"); title(ax,options.Title + " | " + description);
clim(ax,[-limit limit]); cb=colorbar(ax); ylabel(cb,"Re(U)");
