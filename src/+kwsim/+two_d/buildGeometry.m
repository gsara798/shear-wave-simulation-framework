...
% by the run adapter. The 2D geometry rasterizer supports circles and
% axis-aligned rectangles; unknown object types fail
...
for index = 1:numel(objects)
    ...
    switch lower(string(object.type))
        case "circle"
            ...
            requested_bounds_m_xz = [ ...
                object.center_m_xz(1)-object.radius_m, ...
                object.center_m_xz(1)+object.radius_m, ...
                object.center_m_xz(2)-object.radius_m, ...
                object.center_m_xz(2)+object.radius_m];
        case "rectangle"
            if ~isfield(object, 'bounds_m_xz') || ...
                    ~isnumeric(object.bounds_m_xz) || ...
                    numel(object.bounds_m_xz) ~= 4 || ...
                    any(~isfinite(object.bounds_m_xz)) || ...
                    object.bounds_m_xz(2) <= object.bounds_m_xz(1) || ...
                    object.bounds_m_xz(4) <= object.bounds_m_xz(3)
                error('kwsim:InvalidRectangleBounds', ...
                    ['Rectangle bounds must be finite ' ...
                     '[x_min,x_max,z_min,z_max] with positive extent.']);
            end
            requested_bounds_m_xz = double(object.bounds_m_xz);
            coordinate_tolerance = 10*eps(max([x_m(end), z_m(end), 1]));
            mask = X_m >= requested_bounds_m_xz(1)-coordinate_tolerance & ...
                X_m <= requested_bounds_m_xz(2)+coordinate_tolerance & ...
                Z_m >= requested_bounds_m_xz(3)-coordinate_tolerance & ...
                Z_m <= requested_bounds_m_xz(4)+coordinate_tolerance;
            requested_area_m2 = ...
                (requested_bounds_m_xz(2)-requested_bounds_m_xz(1)) * ...
                (requested_bounds_m_xz(4)-requested_bounds_m_xz(3));
        otherwise
    ...
    if isfield(object, 'center_m_xz'), info.center_m_xz = object.center_m_xz; end
    if isfield(object, 'radius_m'), info.radius_m = object.radius_m; end
    info.requested_bounds_m_xz = requested_bounds_m_xz;
    if any(mask, 'all')
        info.realized_bounds_m_xz = [min(X_m(mask)), max(X_m(mask)), ...
            min(Z_m(mask)), max(Z_m(mask))];
    end
...
info = struct('type', "", 'name', "", 'center_m_xz', [NaN, NaN], ...
    ...
    'requested_bounds_m_xz', [NaN, NaN, NaN, NaN], ...
    'realized_bounds_m_xz', [NaN, NaN, NaN, NaN], ...
