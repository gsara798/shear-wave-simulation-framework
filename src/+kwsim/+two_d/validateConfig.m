...
if ~uses_vibrator_bank
    ...
    if single_contact_model == "finite_segment"
        single_profile = lower(string(cfg.source.contact_profile));
        uniform_profile = single_profile == "uniform";

        addCheck("single_contact_profile", ...
            uniform_profile, ...
            double(uniform_profile), ...
            1, ...
            "Single finite contacts currently use a uniform prescribed " + ...
...
for object_index = 1:numel(objects)
    ...
    elseif lower(string(object.type)) == "rectangle"
        bounds = double(object.bounds_m_xz);
        bounds_valid = numel(bounds) == 4 && all(isfinite(bounds)) && ...
            bounds(2) > bounds(1) && bounds(4) > bounds(3);
        addCheck(prefix + "rectangle_bounds", bounds_valid, ...
            double(bounds_valid), 1, ...
            "Rectangle bounds must be finite [x_min,x_max,z_min,z_max].");
        boundary_ok = bounds_valid && bounds(1) >= clearance && ...
            bounds(2) <= x_m(end)-clearance && ...
            bounds(3) >= clearance && bounds(4) <= z_m(end)-clearance;
        addCheck(prefix + "boundary_clearance", boundary_ok, ...
            double(boundary_ok), 1, ...
            "Rectangle violates the physical-domain boundary clearance.");
        if cfg.geometry.require_objects_inside_sensor_roi
            roi_contains_object = boundary_ok && ...
                bounds(1) >= x_m(x_start) && bounds(2) <= x_m(x_end) && ...
                bounds(3) >= z_m(z_start) && bounds(4) <= z_m(z_end);
            addCheck(prefix + "inside_sensor_roi", roi_contains_object, ...
                double(roi_contains_object), 1, ...
                "Rectangle is not fully contained in the sensor ROI.");
        end
    end
