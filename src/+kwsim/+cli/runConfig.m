...
[requested_cfg, config_metadata] = kwsim.io.loadConfigJson(config_file);
requested_cfg = resolveProjectPaths(requested_cfg);

if strlength(string(options.OutputDirectory)) > 0
    requested_cfg.output.directory = string(options.OutputDirectory);
end

[resolved_cfg, preflight] = validateDimensionConfig(requested_cfg);
printConfigurationSummary(resolved_cfg, config_metadata.config_file, options.DryRun);
...
fail_on_invalid = logical(requested_cfg.diagnostics.fail_on_invalid);
execution_cfg = requested_cfg;
execution_cfg.diagnostics.fail_on_invalid = false;

[result, report] = executeDimensionConfig(execution_cfg);

result.config_requested = requested_cfg;
result.config_resolved.diagnostics.fail_on_invalid = fail_on_invalid;
...
wavefield_sample_requested = outputFlag(result.config_resolved, "save_wavefield_sample", false);

if wavefield_sample_requested
    wavefield_sample = kwsim.samples.buildWavefieldSample( ...
        result, Quantity=resolveWavefieldSampleQuantity(result.config_resolved));
end
...
if outputFlag(result.config_resolved, "enabled", false)
    paths = kwsim.io.saveSimulationResult(result);
    validation_paths = kwsim.io.saveValidationReport( ...
        report, paths, ...
        Overwrite=outputFlag(result.config_resolved, "overwrite", false));

    if wavefield_sample_requested && isfield(wavefield_sample, "schema_name")
        wavefield_sample_path = kwsim.samples.saveWavefieldSample( ...
            wavefield_sample, paths, ...
            Overwrite=outputFlag(result.config_resolved, "overwrite", false));
        fprintf("Wavefield sample:\n%s\n", wavefield_sample_path);
    end

    if outputFlag(result.config_resolved, "save_figures", false)
        figure_paths = saveConfiguredFigures(result, report, paths);
    end
...
if ~report.valid && fail_on_invalid
    ...
    error("kwsim:ConfiguredRunValidationFailed", ...
        "Simulation was saved but failed validation: %s", failed_names);
end
...
if isfield(cfg, "wavefield_sample") && isfield(cfg.wavefield_sample, "quantity")
...
function [resolved_cfg, preflight] = validateDimensionConfig(cfg)
switch double(cfg.dimension)
    case 2
        [resolved_cfg, preflight] = kwsim.two_d.validateConfig(cfg);
    case 3
        [resolved_cfg, preflight] = kwsim.three_d.validateConfig(cfg);
    otherwise
        error("kwsim:UnsupportedDimension", "Only dimensions 2 and 3 are supported.");
end
end

function [result, report] = executeDimensionConfig(cfg)
switch double(cfg.dimension)
    case 2
...
scenario = lower(string(result.config_resolved.scenario));
validation_kind = kwsim.validation.classify3DValidationScenario(scenario);
switch validation_kind
    ...
    otherwise
        error("kwsim:Unsupported3DValidation", ...
            "No physical 3D validation routine is registered for scenario '%s'.", scenario);
end
end

function figure_paths = saveConfiguredFigures(result, report, paths)
switch double(result.dimension)
    case 2
        figure_paths = kwsim.viz.plotRun(result, report, paths.figures);
    case 3
        figure_title = "3D shear-wave field";
        if isfield(report, "scope")
            switch string(report.scope)
                case "homogeneous_directional_harmonic_3d"
                    figure_title = "3D directional shear-wave field";
                case "homogeneous_multi_source_harmonic_3d"
                    figure_title = "3D partial-diffuse shear-wave field";
                case "heterogeneous_harmonic_3d"
                    figure_title = "3D heterogeneous shear-wave field";
            end
        end

        handles = kwsim.viz.plotHarmonicVolumeSlices( ...
            result.fields.harmonic_velocity.z_shear_zyx, ...
            result.axes.x_m, result.axes.y_m, result.axes.z_m, ...
            Title=figure_title, AmplitudeScale="normalized", FigureVisible="off");
        cleanup = onCleanup(@() closeFigureIfOpen(handles.figure));
        figure_paths = kwsim.io.saveFigure( ...
            handles.figure, paths, "z_shear_slices", ...
            SaveMatlabFigure=outputFlag(result.config_resolved, "save_matlab_figures", false), ...
            Overwrite=outputFlag(result.config_resolved, "overwrite", false));
        clear cleanup

        if isfield(report, "scope") && string(report.scope) == "heterogeneous_harmonic_3d"
            [x_index, y_index, z_index] = resolveMaterialFigureIndices(result.config_resolved);
            material_handles = kwsim.viz.plotMaterialGeometry3D( ...
                result.config_resolved, Title="3D resolved material geometry", ...
                XIndex=x_index, YIndex=y_index, ZIndex=z_index, FigureVisible="off");
            material_cleanup = onCleanup(@() closeFigureIfOpen(material_handles.figure));
            material_figure_paths = kwsim.io.saveFigure( ...
                material_handles.figure, paths, "material_geometry", ...
                SaveMatlabFigure=outputFlag(result.config_resolved, "save_matlab_figures", false), ...
                Overwrite=outputFlag(result.config_resolved, "overwrite", false));
            figure_paths.material_geometry = material_figure_paths;
            clear material_cleanup
        end
    otherwise
        error("kwsim:UnsupportedDimension", ...
            "Cannot save figures for dimension %g.", result.dimension);
end
end

function [x_index, y_index, z_index] = resolveMaterialFigureIndices(cfg)
domain_center_m_xyz = 0.5 * cfg.derived.domain_size_m_xyz;
slice_point_m_xyz = domain_center_m_xyz;
if ~isempty(cfg.geometry.objects)
    slice_point_m_xyz = cfg.geometry.objects(end).center_m_xyz;
elseif cfg.geometry.bilayer.enabled
    slice_point_m_xyz = cfg.geometry.bilayer.interface_point_m_xyz;
end
[~, x_index] = min(abs(cfg.derived.x_full_m - slice_point_m_xyz(1)));
[~, y_index] = min(abs(cfg.derived.y_full_m - slice_point_m_xyz(2)));
[~, z_index] = min(abs(cfg.derived.z_full_m - slice_point_m_xyz(3)));
end

function cfg = resolveProjectPaths(cfg)
repository_root = resolveRepositoryRoot();
if isfield(cfg, "output") && isfield(cfg.output, "directory")
    cfg.output.directory = resolveRelativePath(cfg.output.directory, repository_root);
end
if isfield(cfg, "solver") && isfield(cfg.solver, "kwave_path") && ...
        strlength(string(cfg.solver.kwave_path)) > 0
    cfg.solver.kwave_path = resolveRelativePath(cfg.solver.kwave_path, repository_root);
end
end
...
function printConfigurationSummary(cfg, config_file, dry_run)
fprintf("\n============================================\n");
