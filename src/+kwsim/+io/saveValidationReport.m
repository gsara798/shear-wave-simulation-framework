...
%   - a directory path
%
% For standardized runs the files are written into validation/.
...
if isstruct(destination)
    if isfield(destination,"validation")
        validation_directory = string(destination.validation);
    elseif isfield(destination,"data")
        % Backward-compatible fallback for historical path structures.
        validation_directory = string(destination.data);
    else
        error("kwsim:InvalidOutputPaths", ...
            "Output paths structure is missing validation/data fields.");
    end
else
    validation_directory = string(destination);
end

if ~isfolder(validation_directory)
    mkdir(validation_directory);
end

mat_path = fullfile(validation_directory,"validation_report.mat");
text_path = fullfile(validation_directory,"validation_summary.txt");

assertWritable(mat_path, options.Overwrite);
assertWritable(text_path, options.Overwrite);

validation_report = report; %#ok<NASGU>
...
fprintf(file_id,"%-38s %-6s %-14s %-14s %s\n", ...
    "Check", "Pass", "Value", "Threshold", "Meaning");
for check = report.checks.'
    message = "";
    if isfield(check, "message"), message = string(check.message); end
    fprintf(file_id,"%-38s %-6d %-14.6g %-14.6g %s\n", ...
        char(string(check.name)), logical(check.pass), ...
        double(check.value), double(check.threshold), char(message));
end
end
