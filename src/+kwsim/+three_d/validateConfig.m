...
if ~(islogical(cfg.sensor.save_full_volume) && ...
        isscalar(cfg.sensor.save_full_volume))
    error("kwsim:Invalid3DConfig", ...
        "sensor.save_full_volume must be a logical scalar.");
end
...
if ~memory.within_limit && cfg.execution.fail_on_memory_limit
    error("kwsim:MemoryLimitExceeded", ...
        "Estimated 3D solver and sensor memory %.3f GB exceeds the configured " + ...
        "limit of %.3f GB.", ...
        memory.estimated_total_gb, ...
